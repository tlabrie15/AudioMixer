...
def update_check():
    if updater.check_for_updates(git_url, version):
        new_ver = updater.get_latest_ver(git_url)
        prompt_update(new_ver)
...
def prompt_update(new_ver):
    ...
    prompt.geometry("160x100")
    prompt.configure(bg="white")
    prompt.protocol("WM_DELETE_WINDOW", ignore)
    txt = "Update to v" + new_ver + "?"

    update_label = Label(prompt, text="UPDATE AVAILABLE", bg="white")
    prompt_label = Label(prompt, text=txt, bg="white")
    update_button = Button(prompt, text="Update", command=update, relief="groove", width=10)
    ignore_button = Button(prompt, text="Ignore", command=ignore, relief="groove", width=10)

    update_label.grid(column=0,row=0, columnspan=6)
    prompt_label.grid(column=0, row=1, columnspan=6)
    update_button.grid(column=1,row=2, columnspan=2)
    ignore_button.grid(column=3,row=2, columnspan=2)
